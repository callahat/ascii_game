class Management::KingdomItemsController < ApplicationController
	...
	def list_inventory
		@player_character_items = PlayerCharacterItem.get_page(params[:page], session[:player_character][:id])
	end
	...
	quant = ( params[:player_character_item] ? params[:player_character_item][:quantity].to_i : 0 )
		...
		elsif PlayerCharacterItem.update_inventory(session[:player_character].id,@item.id,-1 * quant) &&
	...
	quant = ( params[:kingdom_item] ? params[:kingdom_item][:quantity].to_i : 0 )
		...
		if quant <= 0
			...
			redirect_to :action => 'remove', :id => params[:id]
					...
					PlayerCharacterItem.update_inventory(session[:player_character].id,@item.id,1 * quant)
			flash[:notice] = quant.to_s + ' ' + @item.name.pluralize + ' moved from the storerooms to your inventory.'
			redirect_to :action => 'list'
		else
...
protected
	#def add_to_inventory(iid,num)
	#	@item = session[:player_character].player_character_items.find_first(:item_id => iid)
	#	if @item.nil?
	#		@item = PlayerCharacterItem.new
	#		@item.player_character_id = session[:player_character][:id]
	#		@item.quantity = num
	#		@item.item_id = iid
	...
	#	else
	#		@item.quantity += num
	#		#if a negative amount was added thereby removign all of this item, destroy the inventory entry
	#		if @item.quantity == 0
	#			@item.destroy
	#		else
	#			@item.update
	...
	#	end
	#end
	#def add_to_storeroom(iid,num)
	#	@item = session[:kingdom].kingdom_items.find_first(:item_id => iid)
	#	if @item.nil?
	#		@item = KingdomItem.new
	#		@item.kingdom_id = session[:kingdom][:id]
	#		@item.number = num
	#		@item.item_id = iid
	#		@item.name = @item.item.name
	#		@item.save
