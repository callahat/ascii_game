...
class BattleTest < ActiveSupport::TestCase
  ...
  test "new creature battles" do
    ...
    assert battle && msg == ""
    assert battle.groups.size == 1, battle.groups.inspect
    ...
    assert battle2 && msg == ""
    assert battle2.groups.size == 1, battle.groups.inspect
  ...
  test "battle a king" do
    ...
    assert battle.npcs.size > 0 && battle.npcs.size < 11, battle.npcs.size.inspect
    ...
    assert !battle2 && msg != "", battle2.inspect
  end
  ...
  test "summon guards" do
    0.upto(20){
      ...
      assert @guards.size > 0 && @guards.size < 6, @guards.size.inspect }
  end
  ...
  test "spread disease from character" do
    ...
    assert @pc.illnesses.size == 1, @pc.illnesses.size.inspect
    assert @pc.illnesses[0].disease == (@fluid_disease)
  end
  ...
  test "fighter killed" do
    ...
    battle1.reload
    assert battle1.enemies.size == enemies - 1, battle1.report.inspect
    assert battle1.gold == cgold
    assert_equal false, Stat.exists?(cstatid)
    assert_equal false, Health.exists?(chealthid)
  end
  ...
  test "physical damage sub" do
    ...
    assert_equal orig_xp + @wimp_c.experience, @pc.experience
  ...
  test "magic damage sub" do
    ...
    battle1.reload
    assert_equal 4, battle1.enemies.size
    ...
    assert_equal 0, battle2.npcs.size
  end
  ...
  test "healing spell" do
    ...
    assert battle.report[@pc.name][0] =~ /cannot cast Healing/, battle.report[@pc.name].inspect
    ...
    battle.report = {}
    battle.cast_healing_spell(@pc, @hp_heal, @pc)
    assert battle.report[@pc.name][0] =~ /level is too low/, battle.report[@pc.name].inspect
    assert @pc.health.HP == 10
    ...
    battle.report = {}
    battle.cast_healing_spell(@pc, @hp_heal, @pc)
    assert battle.report[@pc.name][0] =~ /not enough HP and\/or MP/, battle.report[@pc.name].inspect
    assert @pc.health.HP == 10
    ...
    @pc.reload
    assert @pc.health.HP == 30
    assert @pc.health.MP == 40,@pc.health.MP.inspect
    assert @pc.illnesses.size == 1
    assert battle.report[@pc.name][0] =~ /cast/, battle.report[@pc.name].inspect
    assert battle.report[@pc.name][1] =~ /healing/, battle.report[@pc.name].inspect
  ...
  test "run away" do
    ...
    @wild_foo.reload
    assert @wild_foo.being_fought == orig_fought, @wild_foo.being_fought.inspect
    assert @wild_foo.number_alive == orig_alive, @wild_foo.number_alive.inspect
    assert battle1.enemies.size == 0
    assert !Battle.exists?(battle1.id)
  end
  ...
  test "fighter killed completion of a quest req" do
    ...
    assert @pc.log_quests.find_by_quest_id(@quest.id).creature_kills.first.quantity == 9, @pc.log_quests.find_by_quest_id(@quest.id).creature_kills.first.quantity.inspect
  end
